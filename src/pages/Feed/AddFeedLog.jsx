...
export default function AddFeedLog({activeBaby, user}) {
      ...
      async function handleSubmit(event) {
        ...
        if (feedType === "breast") {
          requestBody.duration = feedDuration;
        } else if (feedType === "bottle") {
          requestBody.volume = feedVolume;
        } else if (feedType === "food") {
          requestBody.food = feedFood;
        } else if (feedType === "medicine") {
          requestBody.medicine = feedMedicine;
        }
      
        try {
          const token = localStorage.getItem("token");
          const response = await fetch("/api/main/feed/add", {
            method: "POST",
            headers: {
